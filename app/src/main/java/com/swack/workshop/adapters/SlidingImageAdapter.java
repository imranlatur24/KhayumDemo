...
public class SlidingImageAdapter extends PagerAdapter {
    ...
    public Object instantiateItem(ViewGroup view, int position) {
        ...
        Picasso.with(context).load(APIUrl.BASE_IMAGE + sliderLists.get(position).getSliderimg_image()).error(R.mipmap.ic_launcher).into(imageView);
        txtTitle.setVisibility(View.GONE);
        txtMobile.setVisibility(View.GONE);
        view.addView(imageLayout, 0);
        System.out.println("pic : "+APIUrl.BASE_IMAGE + sliderLists.get(position).getSliderimg_image());
